...
fun Spinner(
  ...
  Box(modifier = modifier.wrapContentSize(Alignment.TopStart)) {
    ...
    DropdownMenu(
      ...
      modifier = dropDownModifier.background(color = ExchangeCurrenciesTheme.colors.backgroundSecondary)
    ) {
      options.forEachIndexed { index, element ->
        DropdownMenuItem(
          modifier = Modifier.background(color = ExchangeCurrenciesTheme.colors.backgroundSecondary),
          onClick = {
